...
@keyframes footer-heartbeat {
  0%, 100% { transform: scale(1); filter: drop-shadow(0 0 5px color-mix(in srgb, #673CDD 50%, transparent)); }
  15%, 45% { transform: scale(1.2); filter: drop-shadow(0 0 10px color-mix(in srgb, #673CDD 80%, transparent)); }
...
/* Bima Company Theme Aurora Glow */
.footer-aurora {
  background: radial-gradient(
    circle at 50% 50%, 
    color-mix(in srgb, #673CDD 20%, transparent) 0%, 
    color-mix(in srgb, #FBCE04 10%, transparent) 40%, 
    transparent 70%
  );
}
...
const MarqueeItem = () => (
  <div className="flex items-center space-x-12 px-6">
    <span>100% Paperless</span> <span className="text-[#673CDD]/80">✦</span>
    <span>Lightning Fast Claims</span> <span className="text-[#FBCE04]/80">✦</span>
    <span>Zero Hidden Fees</span> <span className="text-[#673CDD]/80">✦</span>
    <span>24/7 Human Support</span> <span className="text-[#FBCE04]/80">✦</span>
    <span>Total Protection</span> <span className="text-[#673CDD]/80">✦</span>
  </div>
);
        ...
        <footer className="fixed bottom-0 left-0 flex h-screen w-full flex-col justify-between overflow-hidden bg-background text-foreground cinematic-footer-wrapper">
          ...
          <div className="relative z-20 w-full pb-8 px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="text-muted-foreground text-[10px] md:text-xs font-semibold tracking-widest uppercase order-2 md:order-1">
              © 2026 Bima Company. All rights reserved.
            </div>

            {/* "Made with Love" Badge */}
            <div className="footer-glass-pill px-6 py-3 rounded-full flex items-center gap-2 order-1 md:order-2 cursor-default border-border/50">
              <span className="text-muted-foreground text-[10px] md:text-xs font-bold uppercase tracking-widest">Protected with</span>
              <span className="animate-footer-heartbeat text-sm md:text-base text-[#673CDD]">❤</span>
              <span className="text-muted-foreground text-[10px] md:text-xs font-bold uppercase tracking-widest">by</span>
              <span className="text-foreground font-black text-xs md:text-sm tracking-normal ml-1">Bima Company</span>
            </div>
            ...
            >
              <ArrowUp className="w-5 h-5 transform group-hover:-translate-y-1.5 transition-transform duration-300 text-[#673CDD]" />
            </MagneticButton>
